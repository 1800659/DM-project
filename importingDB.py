...
############## PLAYERS -> CLUB #####################
    ...
    #         rel_query ='MATCH (p:Player), (c:Clubs) WHERE p.ID="' + p + '" AND c.ID="' + pl[p]['current_club_id'] + '" CREATE (p)-[:MEMBER_OF]->(c)'
...
############## PARTECIPATION -> CLUB #####################
    ...
    for p in pr :
        if pr[p]['club_id'] != '' and pr[p]['club_id'] in cl.keys():
            rel_query ='MATCH (p:Partecipation), (c:Clubs) WHERE p.ID="' + p + '" AND c.ID="' + pr[p]['club_id'] + '" CREATE (p)-[:PARTECIPATE_IN]->(c)'
            session.run(rel_query)
            club_counter1 += 1
...
############## APPEARANCES -> GAMES #####################
    ...
    for a in app :
        if app[a]['game_id'] != '' and app[a]['game_id'] in gm.keys():
            rel_query ='MATCH (a:Appearances), (g:Game) WHERE a.ID="' + a + '" AND g.ID="' + app[a]['game_id'] + '" CREATE (a)-[:APPEREAD_IN]->(g)'
            session.run(rel_query)
            counter_game1 += 1
...
############## APPEARANCES -> PLAYER #####################
    ...
    for a in app :
        if app[a]['player_id'] != '' and app[a]['player_id'] in pl.keys():
            rel_query ='MATCH (a:Appearances), (p:Player) WHERE a.ID="' + a + '" AND p.ID="' + app[a]['player_id'] + '" CREATE (a)-[:COLLECT]->(p)'
            session.run(rel_query)
            counter_player += 1
